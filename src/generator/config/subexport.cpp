    ...
    for (Proxy &x: nodes) {
        ...
        switch (x.Type) {
            ...
            case ProxyType::AnyTLS:
                ...
                // sni: 修复逻辑错误，统一使用 x.SNI
                if (!x.SNI.empty()) {
                    singleproxy["sni"] = x.SNI;
                }
                // skip-cert-verify: 默认为 true
                if (!scv.is_undef()) {
                    singleproxy["skip-cert-verify"] = scv.get();
                } else {
                    singleproxy["skip-cert-verify"] = true;
                }
                // alpn: 如果有则使用，否则使用默认值 [h2, http/1.1]
                if (!x.AlpnList.empty()) {
                    for (auto &item: x.AlpnList) {
                        singleproxy["alpn"].push_back(item);
                    }
                } else {
                    singleproxy["alpn"].push_back("h2");
                    singleproxy["alpn"].push_back("http/1.1");
                }
                break;
